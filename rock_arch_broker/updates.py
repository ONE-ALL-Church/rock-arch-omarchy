...
class UpdateManager:
    ...
    def _check_worker(self) -> None:
        ...
        except (OSError, UnicodeError, subprocess.SubprocessError, UpdateError):
            with self._lock:
        ...
        # Install only from status(), which receives the current saved preference.
        # The preference captured when this asynchronous check began may be stale.
    ...
    def _manifest_version(
        ...
        if result.returncode != 0 or len(result.stdout.encode("utf-8")) > 64 * 1024:
            raise UpdateError("update_check_failed")
        try:
            manifest = decode_bounded_json(result.stdout)
