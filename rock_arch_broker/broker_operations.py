...
class BrokerOperations:
    ...
    def __init__(self, broker: Broker) -> None:
        ...
        self._handlers: dict[str, OperationHandler] = {
            ...
            "shortcut_status": self._shortcut,
            "shortcut_install": self._shortcut,
            "shortcut_remove": self._shortcut,
    ...
    def handle(self, raw: dict[str, Any]) -> dict[str, Any]:
        ...
        # Local preferences remain reachable when Rock's CLI access is disabled.
        # This does not expose profiles, credentials, or Rock operations.
        if raw.get("client") == CLI_CLIENT and operation not in {
            "settings_status", "settings_update", "shortcut_status", "shortcut_install", "shortcut_remove",
        }:
            try:
    ...
    def _shortcut(self, raw: dict[str, Any]) -> dict[str, Any]:
        broker = self.broker
        if broker._context is Context.DEV or broker._shortcuts is None:
            shortcut = {
                "state": "unavailable", "editable": False, "managed": False,
                "currentCombo": "", "combo": "SUPER + R",
                "error": "preview_mode" if broker._context is Context.DEV else "source_checkout",
            }
        else:
            if (raw["op"] == "shortcut_remove" and raw.get("confirmed") is True
                    and broker._shortcuts.installed):
                try:
                    broker._profile_store.update_preferences({"showMenuBar": True})
                except ProfileError:
                    shortcut = broker._shortcuts.request("shortcut_status")
                    return broker._ok(shortcut={**shortcut, "error": "icon_restore_failed"})
            shortcut = broker._shortcuts.request(
                raw["op"], combo=raw.get("combo", ""),
                revision=raw.get("revision", ""), confirmed=raw.get("confirmed") is True,
            )
        # Keep shortcut failures local to their controls, not the Rock login flow.
        return broker._ok(shortcut=shortcut)
    ...
    def _settings_status(self, _raw: dict[str, Any]) -> dict[str, Any]:
        try:
            preferences = self.broker._profile_store.preferences()
        except ProfileError as error:
            return self.broker._error(str(error))
        return self.broker._ok(settings={key: preferences[key] for key in EDITABLE_PREFERENCES})

    def _settings_update(self, raw: dict[str, Any]) -> dict[str, Any]:
        requested = raw.get("settings")
        if not isinstance(requested, dict) or not set(requested).issubset(EDITABLE_PREFERENCES):
            return self.broker._error("invalid_preferences")
        if requested.get("showMenuBar") is False:
            shortcut = self._shortcut({"op": "shortcut_status"})["shortcut"]
            if shortcut.get("currentActive") is not True:
                return self.broker._error("shortcut_required")
        response = self._preferences_update({"preferences": requested})
