...
def protocol_schema() -> dict[str, Any]:
    ...
    return {
        ...
        "jobs": {
            "access": "rock-arch jobs access --refresh",
            "run": "rock-arch jobs run SAFE_ID --confirm",
            "preview": "rock-arch jobs run SAFE_ID --dry-run",
            "status": "rock-arch jobs status SAFE_ID",
            "scope": "Existing Jobs search results or exact Scheduled Job entity references from active-profile Recent Links. Discovers the Obsidian Scheduled Job List block and requires page access plus block EDIT permission. Prefers the standard Jobs page; ambiguous placements disable triggering.",
            "confirmation": "Single-use, short-lived draft. Access, placement and job identity are checked again before POST.",
            "completion": "requested means the RunNow request was accepted, not that the job completed. Status is the latest Rock record and cannot prove which request caused a run.",
            "retries": "Never retry an uncertain trigger automatically. Inspect job status/history first.",
        },
        "personalLinks": {
            ...
            "deleteSectionWithLinks": "rock-arch links sections delete SECTION_SAFE_ID --with-links --confirm",
            ...
            "deleteScope": "Own private links and sections only. Populated sections require --with-links, which deletes the section and all its links. Dry run reports linkCount and withLinks without deleting. Confirmation rechecks ownership, contents, and exact fields; changed contents require a new review. No automatic retries.",
            ...
            "input": {"name": "bookmark name, at most 100 UTF-16 units", "url": "URL on the active Rock instance",
                      "safeId": "alternative to URL; prefills a current Rock result or Recent Link", "sectionId": "optional section safeId"},
