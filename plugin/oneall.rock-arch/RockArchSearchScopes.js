...
function options(enabled, available) {
  return [
    ...
    {prefix: "ct", label: "Content Types", category: "Content Channel Types", shortcut: "Alt+Shift+C"},
    {prefix: "c", label: "Content Items", category: "Content Channel Items", shortcut: "Alt+C"}
  ].filter(function(item) {
    return enabled.indexOf(item.category) >= 0 && available.indexOf(item.category) >= 0
  })
}
...
function keyForQuery(value) {
  ...
  if (prefix === "pg" || prefix === "page" || prefix === "pages") return "pg"
